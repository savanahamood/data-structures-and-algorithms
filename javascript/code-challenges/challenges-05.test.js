...
const toLastNames = people => {
  ...
  let arr2=[];
  people.map((e)=>{
  let full=e.firstName+" "+e.lastName
  arr2.push(full)
})
  return arr2
};
...
const addValues = (arr) => {
  ...
  return arr.reduce((e, Value) => e + Value);
};
...
const addPurchases = (arr) => {
  ...
  return arr.reduce((e, value) => e + value.purchasePrice,0);
...
const reversedString = (str) => {
  // Solution code here...
  return [...str].reduce((reversed, _, index) => str[index] + reversed, '');
