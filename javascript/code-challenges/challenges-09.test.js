...
const updateNumbers = (obj) => {
  // Solution code here...
  const formattedPhoneBook = Object.entries(obj).map(([name, phoneNumber]) => `${name}: ${phoneNumber}`);
...
const houseSize = (arr) => {
  ...
  for (let character of arr) {
    ...
    const members = (children?.length || 0) + 1 + (spouse ? 1 : 0);
...
const houseSurvivors = (arr) => {
  ...
  const survivors = [];
  for (let character of arr) {
    const { house, spouse, children } = character;
    const members = (spouse && !deceasedSpouses.includes(spouse)) ? (children ? 1 + children.length : 1) : (children ? children.length : 0);
...
describe('Testing challenge 10', () => {
  test('It should not include any deceased spouses', () => {
    expect(houseSurvivors(characters)[2]).toStrictEqual({ house: 'Lannister', members: 3 });
  });
});
